...
class BaseSolver(tf.keras.Model):
    ...
    def train(self, dataset, checkpointer, total_batches=-1):
...
class CTCDecoderSolver(BaseSolver):
    """ DecoderSolver
    """
    default_config = {
        "inference_type": "asr_ctc",
        "decoder_type": "wfst_decoder",
        "model_avg_num": 1,
        "beam_size": 4,
        "ctc_weight": 0.0,
    ...
    def inference(self, dataset, rank_size=1):
        ...
        for _, samples in enumerate(dataset):
            ...
            num_errs, _ = metric.update_state(validated_preds, samples)
            reports = (
                "predictions: %s\tvalidated_preds:%s\tlabels: %s\terrs: %d\tavg_acc: %.4f\tsec/iter: %.4f"
                % (
                    predictions,
                    validated_preds,
                    samples["output"].numpy(),
